...
TimeSurface::TimeSurface(ros::NodeHandle &nh, ros::NodeHandle nh_private) : nh_(nh) {
...
void TimeSurface::createTimeSurfaceAtTime(const ros::Time &external_sync_time) {
    ...
    for (int y = 0; y < sensor_size_.height; ++y) {
        for (int x = 0; x < sensor_size_.width; ++x) {
            ...
            if (pEventQueueMat_->getMostRecentEventBeforeT(
                    x, y, external_sync_time, &most_recent_event_at_coordXY_before_T)) {
                const ros::Time &most_recent_stamp_at_coordXY =
                    most_recent_event_at_coordXY_before_T.ts;
                if (most_recent_stamp_at_coordXY.toSec() > 0) {
                    ...
                    if (time_surface_mode_ == FORWARD && bCamInfoAvailable_) {
                        Eigen::Matrix<double, 2, 1> uv_rect =
                            precomputed_rectified_points_.block<2, 1>(0,
                                                                      y * sensor_size_.width + x);
                        size_t u_i, v_i;
    ...
    if (time_surface_mode_ == BACKWARD && bCamInfoAvailable_ &&
        time_surface_pub_.getNumSubscribers() > 0) {
        cv_bridge::CvImage cv_image2;
        cv_image2.encoding     = cv_image.encoding;
        cv_image2.header.stamp = external_sync_time;
        cv::remap(cv_image.image, cv_image2.image, undistort_map1_, undistort_map2_,
                  CV_INTER_LINEAR);
...
void TimeSurface::thread(Job &job) {
    ...
    for (size_t y = start_row; y <= end_row; y++)
        for (size_t x = start_col; x <= end_col; x++) {
            dvs_msgs::Event most_recent_event_at_coordXY_before_T;
            if (pEventQueueMat_->getMostRecentEventBeforeT(
                    x, y, job.external_sync_time_, &most_recent_event_at_coordXY_before_T)) {
                const ros::Time &most_recent_stamp_at_coordXY =
                    most_recent_event_at_coordXY_before_T.ts;
                if (most_recent_stamp_at_coordXY.toSec() > 0) {
                    const double dt =
                        (job.external_sync_time_ - most_recent_stamp_at_coordXY).toSec();
                    double polarity = (most_recent_event_at_coordXY_before_T.polarity) ? 1.0 : -1.0;
...
void TimeSurface::cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr &msg) {
    ...
    if (distortion_model_ == "equidistant") {
        cv::fisheye::initUndistortRectifyMap(camera_matrix_, dist_coeffs_, rectification_matrix_,
                                             projection_matrix_, sensor_size, CV_32FC1,
        ...
        cv::initUndistortRectifyMap(camera_matrix_, dist_coeffs_, rectification_matrix_,
    ...
    if (distortion_model_ == "plumb_bob") {
        ...
        ROS_INFO("Undistorted-Rectified Look-Up Table with Distortion model: %s",
