...
namespace esvo_core {
esvo_MVStereo::esvo_MVStereo(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
    : nh_(nh),
      pnh_(nh_private),
      ...
      dpConfigPtr_(new DepthProblemConfig(tools::param(pnh_, "patch_size_X", 25),
      ...
      pc_(new PointCloud()),
      depthFramePtr_(
          new DepthFrame(camSysPtr_->cam_left_ptr_->height_, camSysPtr_->cam_left_ptr_->width_)) {
    // frame id
    ...
    em_.resetParameters(EM_Time_THRESHOLD_, EM_EPIPOLAR_THRESHOLD_, EM_TS_NCC_THRESHOLD_,
                        EM_patch_size_X_, EM_patch_size_Y_, EM_patch_intensity_threshold_,
                        EM_patch_valid_ratio_);
    ...
    sgbm_             = cv::StereoSGBM::create(0, num_disparities_, block_size_, P1_, P2_, -1, 0,
                                               uniqueness_ratio_);
    ...
    ebm_.resetParameters(BM_patch_size_X_, BM_patch_size_Y_, minDisparity, maxDisparity, BM_step_,
                         BM_ZNCC_Threshold_, BM_bUpDownConfiguration_);

    // callbacks functions
    events_left_sub_ = nh_.subscribe<dvs_msgs::EventArray>(
        "events_left", 0,
        boost::bind(&esvo_MVStereo::eventsCallback, this, _1, boost::ref(events_left_)));
    events_right_sub_ = nh_.subscribe<dvs_msgs::EventArray>(
        "events_right", 0,
        boost::bind(&esvo_MVStereo::eventsCallback, this, _1, boost::ref(events_right_)));
    ...
    std::thread MappingThread(&esvo_MVStereo::MappingLoop, this, std::move(mapping_thread_promise_),
                              std::move(reset_future_));
    MappingThread.detach();

    // Dynamic reconfigure
    dynamic_reconfigure_callback_ =
        boost::bind(&esvo_MVStereo::onlineParameterChangeCallback, this, _1, _2);
...
void esvo_MVStereo::MappingLoop(std::promise<void> prom_mapping, std::future<void> future_reset) {
...
//    LOG(INFO) << "-------------- vEMP num: " << vEMP.size();
        ...
        if (msm_ == PURE_EVENT_MATCHING) {
            ...
            // These are to accumulate as much depthFrames as the fusion (in the optiomization)
            // does. This is for the comparison in the paper.
            ...
            // visualization
            //      LOG(INFO) << "-------------- depth point num: " << depthFramePtr_->dMap_->size()
            //      << " " << dqvDepthPoints_.size(); exit(-1);
            std::thread tPublishMappingResult(&esvo_MVStereo::publishMappingResults, this,
                                              depthFramePtr_->dMap_, depthFramePtr_->T_world_frame_,
                                              t);
    ...
    if (msm_ == PURE_BLOCK_MATCHING || msm_ == BM_PLUS_ESTIMATION) {
        ...
        if (bDenoising_) {
            ...
            extractDenoisedEvents(vCloseEventsPtr_left_, vDenoisedEventsPtr_left_, denoising_mask,
                                  PROCESS_EVENT_NUM_);
        ...
        // block matching
        tt_mapping.tic();
        ebm_.createMatchProblem(&TS_obs_, &st_map_,
                                &vDenoisedEventsPtr_left_); // interface for one slice
    ...
    if (msm_ == EM_PLUS_ESTIMATION || msm_ == BM_PLUS_ESTIMATION) {
        ...
        dpSolver_.pointCulling(vdp, stdVar_vis_threshold_, cost_vis_threshold_, invDepth_min_range_,
                               invDepth_max_range_);
        ...
        depthFramePtr_->dMap_->clean(pow(stdVar_vis_threshold_, 2), age_vis_threshold_,
    ...
    LOG(INFO) << "-- fusion (" << TotalNumFusion_ << "): " << t_fusion << " ms, ("
...
bool esvo_MVStereo::dataTransferring() {
    ...
    if (msm_ == PURE_EVENT_MATCHING || msm_ == EM_PLUS_ESTIMATION) {
        ...
        if (ev_left_lowBound == ev_left_upBound ||
            ev_right_lowBound == ev_right_upBound) // no events !!!
            return false;
        ...
        while (ev_left_lowBound != ev_left_upBound &&
               vEventsPtr_left_.size() <= EM_numEventMatching_) {
            vEventsPtr_left_.push_back(ev_left_lowBound._M_cur);
            ev_left_lowBound++;
        }
        while (ev_right_lowBound != ev_right_upBound &&
               vEventsPtr_right_.size() <= EM_numEventMatching_) {
    ...
    if (msm_ == PURE_BLOCK_MATCHING || msm_ == BM_PLUS_ESTIMATION) {
        ...
        auto      ev_end_it = tools::EventBuffer_lower_bound(events_left_, t_end);
        auto      ev_begin_it =
            tools::EventBuffer_lower_bound(events_left_, t_begin); // events_left_.begin();
...
void esvo_MVStereo::stampedPoseCallback(const geometry_msgs::PoseStampedConstPtr &ps_msg) {
    ...
    int                     iGetLastest_common_time      = tf_->getLatestCommonTime(
                                 dvs_frame_id_.c_str(), ps_msg->header.frame_id, tf_lastest_common_time_, err_tf);
    delete err_tf;
    ...
    if (tf_lastest_common_time_.toSec() != 0) {
        ...
        if (dt < 0 || std::fabs(dt) >= max_time_diff_before_reset_s) {
            ROS_INFO("Inconsistent event timestamps detected <stampedPoseCallback> (new: %f, old "
                     "%f), resetting.",
    ...
    // add pose to tf
    tf::Transform tf(
        tf::Quaternion(ps_msg->pose.orientation.x, ps_msg->pose.orientation.y,
                       ps_msg->pose.orientation.z, ps_msg->pose.orientation.w),
        tf::Vector3(ps_msg->pose.position.x, ps_msg->pose.position.y, ps_msg->pose.position.z));
    tf::StampedTransform st(tf, ps_msg->header.stamp, ps_msg->header.frame_id,
                            dvs_frame_id_.c_str());
    tf_->setTransform(st);
}
...
bool esvo_MVStereo::getPoseAt(const ros::Time           &t,
    ...
    if (!tf_->canTransform(world_frame_id_, source_frame, t, err_msg)) {
        ...
        // HARDCODED: The GT pose of rpg dataset is the pose of stereo rig, namely that of the
        // marker.
        if (std::strcmp(source_frame.c_str(), "marker") == 0) {
            Transformation ::TransformationMatrix T_marker_cam;
            T_marker_cam << 5.363262328777285e-01, -1.748374625145743e-02, -8.438296573030597e-01,
                -7.009849865398374e-02, 8.433577587813513e-01, -2.821937531845164e-02,
                5.366109927684415e-01, 1.881333563905305e-02, -3.319431623758162e-02,
                -9.994488408486204e-01, -3.897382049768972e-04, -6.966829200678797e-02, 0, 0, 0, 1;
            Transformation::TransformationMatrix T_world_marker = Tr.getTransformationMatrix();
            Transformation::TransformationMatrix T_world_cam    = T_world_marker * T_marker_cam;
...
void esvo_MVStereo::eventsCallback(const dvs_msgs::EventArray::ConstPtr &msg, EventQueue &EQ) {
    std::lock_guard<std::mutex> lock(data_mutex_);
    ...
    if (!msg->events.empty() && !EQ.empty()) {
        ...
        if (dt < 0 || std::fabs(dt) >= max_time_diff_before_reset_s) {
            ROS_INFO("Inconsistent event timestamps detected <eventCallback> (new: %f, old %f), "
                     "resetting.",
    ...
    for (const dvs_msgs::Event &e : msg->events) {
        EQ.push_back(e);
        int i = EQ.size() - 2;
        while (i >= 0 && EQ[i].ts > e.ts) // we may have to sort the queue, just in case the raw
...
void esvo_MVStereo::timeSurfaceCallback(const sensor_msgs::ImageConstPtr &time_surface_left,
    ...
    if (!TS_history_.empty()) {
        ...
        if (dt < 0 || std::fabs(dt) >= max_time_diff_before_reset_s) {
            ROS_INFO("Inconsistent frame timestamp detected <timeSurfaceCallback> (new: %f, old "
                     "%f), resetting.",
    ...
    if (dpSolver_.getProblemType() == NUMERICAL)
        ...
        TS_history_.emplace(t_new_TS,
                            TimeSurfaceObservation(cv_ptr_left, cv_ptr_right, TS_id_, true));
    TS_id_++;
...
void esvo_MVStereo::onlineParameterChangeCallback(DVS_MappingStereoConfig &config, uint32_t level) {
    bool have_display_parameters_changed = false;
    {
        ...
        if (invDepth_min_range_ != config.invDepth_min_range ||
            ...
            maxNumFusionFrames_ != config.maxNumFusionFrames || bDenoising_ != config.Denoising ||
            bRegularization_ != config.Regularization || resetButton_ != config.ResetButton ||
            ...
            BM_max_disparity_ != config.BM_max_disparity || BM_step_ != config.BM_step ||
...
void esvo_MVStereo::publishMappingResults(DepthMap::Ptr  depthMapPtr,
                                          Transformation tr,
                                          ros::Time      t) {
    cv::Mat invDepthImage, stdVarImage, ageImage, costImage, eventImage, confidenceMap;

    visualizor_.plot_map(depthMapPtr, tools::InvDepthMap, invDepthImage, invDepth_max_range_,
                         invDepth_min_range_, stdVar_vis_threshold_, age_vis_threshold_);
    publishImage(invDepthImage, t, invDepthMap_pub_);

    visualizor_.plot_map(depthMapPtr, tools::StdVarMap, stdVarImage, stdVar_vis_threshold_, 0.0,
                         stdVar_vis_threshold_);
    publishImage(stdVarImage, t, stdVarMap_pub_);

    visualizor_.plot_map(depthMapPtr, tools::AgeMap, ageImage, age_max_range_, 0,
                         age_vis_threshold_);
    publishImage(ageImage, t, ageMap_pub_);

    visualizor_.plot_map(depthMapPtr, tools::CostMap, costImage, cost_vis_threshold_, 0.0,
                         cost_vis_threshold_);
...
void esvo_MVStereo::saveDepthMap(DepthMap::Ptr &depthMapPtr, std::string &saveDir, ros::Time t) {
    std::ofstream of;
...
void esvo_MVStereo::publishPointCloud(DepthMap::Ptr  &depthMapPtr,
                                      Transformation &tr,
                                      ros::Time      &t) {
    ...
    for (auto it = depthMapPtr->begin(); it != depthMapPtr->end(); it++) {
        Eigen::Vector3d p_world =
            T_world_result.block<3, 3>(0, 0) * it->p_cam() + T_world_result.block<3, 1>(0, 3);
...
void esvo_MVStereo::publishImage(const cv::Mat              &image,
                                 const ros::Time            &t,
...
void esvo_MVStereo::vEMP2vDP(std::vector<EventMatchPair> &vEMP, std::vector<DepthPoint> &vdp) {
    vdp.reserve(vEMP.size());
    double var_pseudo = 0; // pow(stdVar_vis_threshold_*0.99,2);// used for being compatible with
                           // the interface, just for visualization.
...
void esvo_MVStereo::eventSlicingForEM(std::vector<EventSlice> &eventSlices) {
    size_t numSlice =
        std::floor((t_upBound_.toSec() - t_lowBound_.toSec()) /
                   EM_Slice_Thickness_); // a small number of events are ignored at this step.
    ...
    for (size_t i = 0; i < numSlice; i++) {
        ...
        if (it_tmp == vEventsPtr_left_.end())
            break;
        //    LOG(INFO) << "num of events in this slice[" << i << "]: " <<
        //    eventSlices[i].numEvents_;
    }
    //  LOG(INFO) << "eventSlices: " << eventSlices.size();
}

void esvo_MVStereo::createEdgeMask(std::vector<dvs_msgs::Event *>         &vEventsPtr,
...
void esvo_MVStereo::createDenoisingMask(std::vector<dvs_msgs::Event *> &vAllEventsPtr,
                                        cv::Mat                        &mask,
                                        size_t                          row,
                                        size_t                          col) {
    cv::Mat eventMap;
...
void esvo_MVStereo::extractDenoisedEvents(std::vector<dvs_msgs::Event *> &vCloseEventsPtr,
