...
void PerspectiveCamera::setIntrinsicParameters(size_t               width,
                                               size_t               height,
...
void PerspectiveCamera::preComputeRectifiedCoordinate() {
    ...
    if (distortion_model_ == "plumb_bob") {
        cv::undistortPoints(RawCoordinates, RectCoordinates, cvKmat, cvDistCoeff, cvRectMat,
                            cvPmat);
#if CV_MAJOR_VERSION >= 3
        cv::Size sensor_size(width_, height_);
        cv::initUndistortRectifyMap(cvKmat, cvDistCoeff, cvRectMat, cvPmat, sensor_size, CV_32FC1,
...
//    LOG(INFO) << "#################### UndistortRectify_mask_.size: " <<
//    UndistortRectify_mask_.size();
#else
        ...
        cv::fisheye::undistortPoints(RawCoordinates, RectCoordinates, cvKmat, cvDistCoeff,
                                     cvRectMat, cvPmat);
#if CV_MAJOR_VERSION >= 3
        cv::Size sensor_size(width_, height_);
        cv::fisheye::initUndistortRectifyMap(cvKmat, cvDistCoeff, cvRectMat, cvPmat, sensor_size,
...
Eigen::Matrix<double, 2, 1> PerspectiveCamera::getRectifiedUndistortedCoordinate(int xcoor,
                                                                                 int ycoor) {
    size_t index = ycoor * width_ + xcoor;
    return precomputed_rectified_points_.block<2, 1>(0, index);
}

void PerspectiveCamera::cam2World(const Eigen::Vector2d &x, double invDepth, Eigen::Vector3d &p) {
...
void PerspectiveCamera::world2Cam(const Eigen::Vector3d &p, Eigen::Vector2d &x) {
...
void CameraSystem::computeBaseline() {
    Eigen::Vector3d temp =
        cam_right_ptr_->P_.block<3, 3>(0, 0).inverse() * cam_right_ptr_->P_.block<3, 1>(0, 3);
    baseline_ = temp.norm();
}
