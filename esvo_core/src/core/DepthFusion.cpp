...
namespace esvo_core {
namespace core {
DepthFusion::DepthFusion(CameraSystem::Ptr                   &camSysPtr,
                         std::shared_ptr<DepthProblemConfig> &dpConfigPtr)
    : camSysPtr_(camSysPtr), dpConfigPtr_(dpConfigPtr) {}

DepthFusion::~DepthFusion() {}

bool DepthFusion::propagate_one_point(DepthPoint                  &dp_prior,
                                      DepthPoint                  &dp_prop,
                                      Eigen::Matrix<double, 4, 4> &T_prop_prior) {
    Eigen::Vector3d p_prop =
        T_prop_prior.block<3, 3>(0, 0) * dp_prior.p_cam() + T_prop_prior.block<3, 1>(0, 3);

    Eigen::Vector2d x_prop;
    camSysPtr_->cam_left_ptr_->world2Cam(p_prop, x_prop);
    if (!boundaryCheck(x_prop(0), x_prop(1), camSysPtr_->cam_left_ptr_->width_,
    ...
    double denominator =
        T_prop_prior.block<1, 2>(2, 0) * dp_prior.p_cam().head(2) + T_prop_prior(2, 3);
...
int DepthFusion::update(std::vector<DepthPoint> &dp_obs, DepthFrame::Ptr &df, int fusion_radius) {
    int                         numFusion = 0;
    Eigen::Matrix<double, 4, 4> T_frame_world =
        df->T_world_frame_.inverse().getTransformationMatrix();
...
int DepthFusion::fusion(DepthPoint &dp_prop, DepthMap::Ptr &dm, int fusion_radius) {
    int numFusion = 0;
    ...
    for (size_t i = 0; i < vpCoordinate.size(); i++) {
        ...
        if (!boundaryCheck(col, row, camSysPtr_->cam_left_ptr_->width_,
                           camSysPtr_->cam_left_ptr_->height_))
            continue;
        ...
        if (!dm->exists(row, col)) {
            ...
            if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0)
                ...
                dp_new.update_studentT(dp_prop.invDepth(), dp_prop.scaleSquared(),
                                       dp_prop.variance(), dp_prop.nu());
            } else
                exit(-1);
        ...
        } else // case 2: occupied
        {
            ...
            if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0)
                ...
                bCompatibility =
                    studentTCompatibleTest(dp_prop.invDepth(), dm->at(row, col).invDepth(),
                                           dp_prop.variance(), dm->at(row, col).variance());
            } else
                exit(-1);
            ...
            if (bCompatibility) {
                if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0)
                    ...
                    dm->get(row, col).update_studentT(dp_prop.invDepth(), dp_prop.scaleSquared(),
                                                      dp_prop.variance(), dp_prop.nu());
                else
                    exit(-1);

                dm->get(row, col).age()++;
                dm->get(row, col).residual() =
                    min(dm->get(row, col).residual(), dp_prop.residual());
                Eigen::Vector3d p_update;
                camSysPtr_->cam_left_ptr_->cam2World(dm->get(row, col).x(), dp_prop.invDepth(),
                                                     p_update);
                dm->get(row, col).update_p_cam(p_update);
                numFusion++;
            } else // case 2.2 not compatible
            {
                // consider occlusion (the pixel is already assigned with a point that is closer to
                // the camera)
                if (dm->at(row, col).invDepth() - 2 * sqrt(dm->at(row, col).variance()) >
                    dp_prop.invDepth())
                    continue;
                if (dp_prop.variance() < dm->at(row, col).variance() &&
                    dp_prop.residual() <
                        dm->at(row, col).residual()) //&& other requirement? such as cost?
                {
...
bool DepthFusion::boundaryCheck(double xcoor, double ycoor, size_t width, size_t height) {
...
bool DepthFusion::chiSquareTest(double invD1, double invD2, double var1, double var2) {
...
bool DepthFusion::studentTCompatibleTest(double invD1, double invD2, double var1, double var2) {
...
void DepthFusion::naive_propagation(std::vector<DepthPoint> &dp_obs, DepthFrame::Ptr &df) {
    Eigen::Matrix<double, 4, 4> T_frame_world =
        df->T_world_frame_.inverse().getTransformationMatrix();
    for (size_t i = 0; i < dp_obs.size(); i++) {
        Eigen::Matrix<double, 4, 4> T_frame_obs = T_frame_world * dp_obs[i].T_world_cam();
...
bool DepthFusion::naive_propagate_one_point(DepthPoint                  &dp_prior,
                                            DepthPoint                  &dp_prop,
                                            Eigen::Matrix<double, 4, 4> &T_prop_prior) {
    Eigen::Vector3d p_prop =
        T_prop_prior.block<3, 3>(0, 0) * dp_prior.p_cam() + T_prop_prior.block<3, 1>(0, 3);
