...
void DepthProblem::setProblem(Eigen::Vector2d             &coor,
    ...
    Eigen::Matrix<double, 4, 4> T_left_world =
        pStampedTsObs_->second.tr_.inverse().getTransformationMatrix();
    ...
    if (!warping(coordinate_, x(0), vT_left_virtual_[0], x1_s, x2_s)) {
        if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0)
            ...
            for (size_t i = 0; i < patchSize; i++) {
                double residual = 255;
                double weight =
                    (dpConfigPtr_->td_nu_ + 1) /
                    (dpConfigPtr_->td_nu_ + std::pow(residual / dpConfigPtr_->td_scale_, 2));
        ...
        if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0) {
            ...
            for (size_t y = 0; y < wy; y++)
                for (size_t x = 0; x < wx; x++) {
                    ...
                    fvec[index] = ((tau1(y, x) - mu1) / sigma1 - (tau2(y, x) - mu2) / sigma2) /
                                  sqrt(patchSize);
                }
        } else if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "Tdist") == 0) {
            ...
            while (fabs(scaleSquaredTmp2 - scaleSquaredTmp1) / scaleSquaredTmp1 > 0.05 ||
                   first_iteration) {
                if (!first_iteration)
                ...
                for (size_t y = 0; y < wy; y++) {
                    for (size_t x = 0; x < wx; x++) {
                        ...
                        if (vResidual[index] != 0)
                            sum_scaleSquared +=
                                vResidualSquared[index] * (dpConfigPtr_->td_nu_ + 1) /
            ...
            for (size_t y = 0; y < wy; y++) {
                for (size_t x = 0; x < wx; x++) {
                    ...
                    double weight =
                        (dpConfigPtr_->td_nu_ + 1) /
                        (dpConfigPtr_->td_nu_ + vResidualSquared[index] / scaleSquaredTmp2);
...
bool DepthProblem::warping(const Eigen::Vector2d             &x,
    ...
    Eigen::Vector3d p_left =
        T_left_virtual.block<3, 3>(0, 0) * p_rv + T_left_virtual.block<3, 1>(0, 3);
