...
esvo_core::core::EventBM::EventBM(esvo_core::CameraSystem::Ptr camSysPtr,
                                  size_t                       numThread,
                                  bool                         bSmoothTS,
                                  size_t                       patch_size_X,
                                  size_t                       patch_size_Y,
...
void esvo_core::core::EventBM::resetParameters(size_t patch_size_X,
                                               size_t patch_size_Y,
                                               size_t min_disparity,
                                               size_t max_disparity,
                                               size_t step,
...
void esvo_core::core::EventBM::createMatchProblem(StampedTimeSurfaceObs          *pStampedTsObs,
...
bool esvo_core::core::EventBM::match_an_event(dvs_msgs::Event                 *pEvent,
    ...
    Eigen::Vector2d x_rect =
        camSysPtr_->cam_left_ptr_->getRectifiedUndistortedCoordinate(pEvent->x, pEvent->y);
    // check if the rectified and undistorted coordinates are outside the image plane. (Added by Yi
    // Zhou on 12 Jan 2021)
    ...
    // coarse searching
    if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst, lowDisparity, upDisparity,
                           step_, x1, patch_src, bUpDownConfiguration_)) {
    ...
    if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst, fine_searching_start_pos,
                           bestDisp + (step_ - 1), 1, x1, patch_src, bUpDownConfiguration_)) {
        // This indicates the local minima is not surrounded by two neighbors with larger cost,
        // This case happens when the best match locates over/outside the boundary of the Time
        // Surface.
...
bool esvo_core::core::EventBM::epipolarSearching(double          &min_cost,
                                                 Eigen::Vector2i &bestMatch,
                                                 size_t          &bestDisp,
                                                 Eigen::MatrixXd &patch_dst,
                                                 size_t           searching_start_pos,
                                                 size_t           searching_end_pos,
                                                 size_t           searching_step,
                                                 Eigen::Vector2i &x1,
    ...
    for (size_t disp = searching_start_pos; disp <= searching_end_pos; disp += searching_step) {
        ...
        patch_dst   = pStampedTsObs_->second.TS_right_.block(x2_left_top(1), x2_left_top(0),
    ...
    if (searching_step > 1) // coarse
    {
        if (mDispCost.find(bestDisp - searching_step) != mDispCost.end() &&
            ...
            if (mDispCost[bestDisp - searching_step] < ZNCC_MAX_ &&
                mDispCost[bestDisp + searching_step] < ZNCC_MAX_)
                if (min_cost < ZNCC_Threshold_)
                    bFoundOneMatch = true;
            //      else
            //        LOG(INFO) << "coarse searching fails: " << mDispCost[bestDisp -
            //        searching_step] << " <-> "
...
void esvo_core::core::EventBM::match_all_SingleThread(std::vector<EventMatchPair> &vEMP) {
    coarseSearchingFailNum_ = 0;
    fineSearchingFailNum_   = 0;
    infoNoiseRatioLowNum_   = 0;
...
bool esvo_core::core::EventBM::isValidPatch(Eigen::Vector2i &x, Eigen::Vector2i &left_top) {
...
double esvo_core::core::EventBM::zncc_cost(Eigen::MatrixXd &patch_left,
    ...
    if (!normalized) {
        ...
        cost = 0.5 * (1 - (patch_left.array() * patch_right.array()).sum() /
                              (patch_left.rows() * patch_left.cols()));
    return cost;
}