...
namespace esvo_core {
namespace core {
DepthProblemSolver::DepthProblemSolver(CameraSystem::Ptr                   &camSysPtr,
                                       std::shared_ptr<DepthProblemConfig> &dpConfigPtr,
                                       DepthProblemType                     dpType,
                                       size_t                               numThread)
    : camSysPtr_(camSysPtr), dpConfigPtr_(dpConfigPtr), dpType_(dpType), NUM_THREAD_(numThread) {}
...
void DepthProblemSolver::solve(std::vector<EventMatchPair> *pvEMP,
    ...
    for (size_t i = 0; i < NUM_THREAD_; i++) {
        ...
        if (dpType_ == NUMERICAL)
            jobs[i].numDiff_dProblemPtr_ =
                std::make_shared<Eigen::NumericalDiff<DepthProblem>>(dpConfigPtr_, camSysPtr_);
        else if (dpType_ == ANALYTICAL)
            jobs[i].dProblemPtr_ = std::make_shared<DepthProblem>(
                dpConfigPtr_,
                camSysPtr_); // NOTE: The ANALYTICAL version is not provided in this version.
        else {
    ...
    for (size_t i = 0; i < NUM_THREAD_; i++)
        threads.emplace_back(
            std::bind(&DepthProblemSolver::solve_multiple_problems, this, jobs[i]));
...
void DepthProblemSolver::solve_multiple_problems(Job &job) {
    ...
    for (size_t i = i_thread; i < numEvent; i += NUM_THREAD_) {
        ...
        Eigen::Matrix<double, 4, 4> T_world_virtual =
            (*job.pvEMP_)[i].trans_.getTransformationMatrix();
        ...
        if (dpType_ == NUMERICAL) {
            ...
            bProblemSolved =
                solve_single_problem_numerical(d_init, job.numDiff_dProblemPtr_, result);
        } else if (dpType_ == ANALYTICAL) {
...
void DepthProblemSolver::pointCulling(std::vector<DepthPoint> &vdp,
    ...
    for (size_t i = 0; i < vdp.size(); i++) {
        if (vdp[i].variance() <= pow(std_variance_threshold, 2) &&
            vdp[i].residual() <= cost_threshold && vdp[i].valid() &&
            vdp[i].invDepth() >= invDepth_min_range && vdp[i].invDepth() <= invDepth_max_range) {
