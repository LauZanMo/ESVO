...
namespace esvo_core {
namespace core {
DepthRegularization::DepthRegularization(std::shared_ptr<DepthProblemConfig> &dpConfigPtr) {
    ...
    _regularizationMinCloseNeighbours =
        dpConfigPtr_->RegularizationMinCloseNeighbours_; // 32;//8;//16
}
    ...
    while (it != dm.end()) {
        ...
        if (it->valid()) {
            ...
            if (neighbours.size() > _regularizationMinNeighbours) {
                ...
                // regularizationMinCloseNeighbours is larger than the fusion's applied region
                // (namely 4 pixels in my implementation)
                    ...
                    if (strcmp(dpConfigPtr_->LSnorm_.c_str(), "l2") == 0) {
                        ...
                        for (size_t i = 0; i < closeNeighbours.size(); i++)
                            statisticalMean += closeNeighbours[i]->invDepth() *
                                               (1.0 / closeNeighbours[i]->variance()) /
                                               totalInvVariances;
                        ...
                        for (size_t i = 1; i < closeNeighbours.size(); i++) {
                            ...
                            invDepth_post =
                                (scale2_obs * invDepth_prior + scale2_prior * invDepth_obs) /
                                (scale2_obs + scale2_prior);
                            scale2_post = (nu_post + pow(invDepth_prior - invDepth_obs, 2) /
                                                         (scale2_prior + scale2_obs)) /
                                          (nu_post + 1) * (scale2_prior * scale2_obs) /
                                          (scale2_prior + scale2_obs);
                        }
