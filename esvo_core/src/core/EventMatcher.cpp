...
// [26] S.-H. Ieng, J. Carneiro, M. Osswald, and R. Benosman, “Neuromorphic event-based generalized
// time-based stereovision,” Front. Neurosci., vol. 12, p. 442, 2018. Note that the photometric
// consistency check is not applicable because corresponding brightness information is not available
// for DVS sensors.
#include <esvo_core/core/EventMatcher.h>
#include <esvo_core/tools/TicToc.h>
#include <thread>

esvo_core::core::EventMatcher::EventMatcher(CameraSystem::Ptr camSysPtr,
                                            size_t            numThread,
...
void esvo_core::core::EventMatcher::resetParameters(double Time_THRESHOLD,
                                                    double EPIPOLAR_THRESHOLD,
                                                    double TS_NCC_THRESHOLD,
                                                    size_t patch_size_X,
                                                    size_t patch_size_Y,
...
bool esvo_core::core::EventMatcher::match_an_event(dvs_msgs::Event *ev_ptr,
                                                   Transformation  &Trans_world_rv,
                                                   EventMatchPair  &emPair) {
    std::vector<std::vector<dvs_msgs::Event *>::iterator> candidates_time_check,
        candidates_epipolar_check, candidates_motion_check;
    //--- temporal check
    ros::Time time_lowBound(ev_ptr->ts.toSec() - Time_THRESHOLD_ / 2);
    ros::Time time_upBound(ev_ptr->ts.toSec() + Time_THRESHOLD_ / 2);
    std::vector<dvs_msgs::Event *>::iterator candidate_begin_it =
        EventVecPtr_lower_bound(*pvCandEventPtr_, time_lowBound);
    std::vector<dvs_msgs::Event *>::iterator candidate_end_it =
        EventVecPtr_lower_bound(*pvCandEventPtr_, time_upBound);
    ...
    //--- epipolar check
    Eigen::Vector2d x_left =
        camSysPtr_->cam_left_ptr_->getRectifiedUndistortedCoordinate(ev_ptr->x, ev_ptr->y);
    ...
    for (size_t i = 0; i < candidates_time_check.size(); i++) {
        ...
        if (fabs(x_left(1) - x_right(1)) <= EPIPOLAR_THRESHOLD_ && x_right(0) < x_left(0)) {
...
void esvo_core::core::EventMatcher::match_all_SingleThread(std::vector<EventMatchPair> &vEMP) {
...
void esvo_core::core::EventMatcher::match_all_HyperThread(vector<EventMatchPair> &vEMP) {
    ...
    vEMP.clear();
    for (size_t i = 0; i < NUM_THREAD_; i++)
        vEMP.insert(vEMP.end(), jobs[i].pvEventMatchPair_->begin(),
                    jobs[i].pvEventMatchPair_->end());
}

void esvo_core::core::EventMatcher::match(EventMatcher::Job &job) {
    TicToc tt;
    tt.tic();
    ...
    for (size_t i = i_thread; i < totalNumEvents;
         i += NUM_THREAD_, std::advance(ev_it, NUM_THREAD_)) {
...
double esvo_core::core::EventMatcher::zncc_cost(Eigen::MatrixXd &patch_left,
    ...
    double cost =
        0.5 * (1 - (patch_left_sub_normalized.array() * patch_right_sub_normalized.array()).sum());
    //  LOG(INFO) << "cost: " << cost;
    return cost;
}

bool esvo_core::core::EventMatcher::warping2(const Eigen::Vector2d             &x,
                                             double                             invDepth,
    ...
    if (x1_s(0) < (wx - 1) / 2 || x1_s(0) > width - (wx - 1) / 2 || x1_s(1) < (wy - 1) / 2 ||
        x1_s(1) > height - (wy - 1) / 2)
        return false;
    if (x2_s(0) < (wx - 1) / 2 || x2_s(0) > width - (wx - 1) / 2 || x2_s(1) < (wy - 1) / 2 ||
        x2_s(1) > height - (wy - 1) / 2)
        return false;
    return true;
}

bool esvo_core::core::EventMatcher::patchInterpolation2(const Eigen::MatrixXd &img,
