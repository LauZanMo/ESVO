...
esvo_Mapping::esvo_Mapping(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
    ...
    events_left_sub_ = nh_.subscribe<dvs_msgs::EventArray>(
        "events_left", 0,
        boost::bind(&esvo_Mapping::eventsCallback, this, _1, boost::ref(events_left_)));
    events_right_sub_ = nh_.subscribe<dvs_msgs::EventArray>(
        "events_right", 0,
        boost::bind(&esvo_Mapping::eventsCallback, this, _1, boost::ref(events_right_)));
    ...
    std::thread MappingThread(&esvo_Mapping::MappingLoop, this, std::move(mapping_thread_promise_),
                              std::move(reset_future_));
    MappingThread.detach();

    // Dynamic reconfigure
    dynamic_reconfigure_callback_ =
        boost::bind(&esvo_Mapping::onlineParameterChangeCallback, this, _1, _2);
...
void esvo_Mapping::MappingLoop(std::promise<void> prom_mapping, std::future<void> future_reset) {
        ...
        if (TS_history_.size() >= 10) /* To assure the esvo_time_surface node has been working. */
        {
            ...
            if (ESVO_System_Status_ == "INITIALIZATION" || ESVO_System_Status_ == "RESET") {
                ...
                if (InitializationAtTime(TS_obs_.first)) {
                    ...
                    LOG(INFO)
                        << "Initialization is successfully done!"; //(" << INITIALIZATION_COUNTER_
                                                                   //<< ").";
#ifdef ESVO_CORE_MAPPING_DEBUG
...
void esvo_Mapping::MappingAtTime(const ros::Time &t) {
    ...
    size_t numFusionCount = 0; // To count the total number of fusion (in terms of fusion between
    ...
    // Now let's play gently. We do not clean the resulting depth map at the beginning (as long as
    // the number of "frames" to be fused is less than maxNumFusionFrames_), because the depth
    // estimates given by SGM (initialization) would not be fused if the T-distribution statistics
    // is used. Therefore, to guarantee a stable tracking at the beginning, we have to preserve as
    // much depth points as possible. Note that, typically, most depth estimates got cleaned because
    // of their "age", which is an attribute reflecting the "goodness of fit" besides standard
    // variance.
...
bool esvo_Mapping::InitializationAtTime(const ros::Time &t) {
    ...
    sgbm_->compute(TS_obs_.second.cvImagePtr_left_->image, TS_obs_.second.cvImagePtr_right_->image,
                   dispMap);
    ...
    createEdgeMask(vEventsPtr_left_SGM_, camSysPtr_->cam_left_ptr_, edgeMap, vEdgeletCoordinates,
                   true, 0);
...
bool esvo_Mapping::dataTransferring() {
    ...
    if (ESVO_System_Status_ == "WORKING") {
        ...
        vALLEventsPtr_left_.clear();   // Used to generate denoising mask (only used to deal with
                                       // flicker induced by VICON.)
        ...
        // We made a trade off by assuming that events occurred within (0.05 *
        // BM_half_slice_thickness_) ms share an identical pose (virtual view). Here we load
        // transformations for all virtual views.
        st_map_.clear();
...
void esvo_Mapping::stampedPoseCallback(const geometry_msgs::PoseStampedConstPtr &ps_msg) {
    ...
    //    tf_->getLatestCommonTime(dvs_frame_id_.c_str(), ps_msg->header.frame_id,
    //    tf_lastest_common_time_, err_tf);
    delete err_tf;
...
void esvo_Mapping::eventsCallback(const dvs_msgs::EventArray::ConstPtr &msg, EventQueue &EQ) {
    std::lock_guard<std::mutex> lock(data_mutex_);
...
void esvo_Mapping::publishPointCloud(DepthMap::Ptr &depthMapPtr, Transformation &tr, ros::Time &t) {
