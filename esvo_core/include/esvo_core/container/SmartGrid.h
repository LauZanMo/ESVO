...
template <class T>
SmartGrid<T> &SmartGrid<T>::operator=(const SmartGrid &rhs) {
    // copy grid elements
...
template <class T>
void SmartGrid<T>::resize(size_t rows, size_t cols) {
    this->reset();
    _grid.reserve(rows);
    for (size_t r = 0; r < rows; r++) {
        _grid.push_back(new std::vector<T>());
...
template <class T>
void SmartGrid<T>::erode(size_t radius, size_t border, double ratio) {
    ...
    while (it != gridTmp._elements.end()) {
        ...
        for (int r = it->row() - radius; r < it->row() + radius + 1; r++) {
            for (int c = it->col() - radius; c < it->col() + radius + 1; c++) {
                ...
                if (r >= border && r < (int)rows() - border && c >= border &&
                    c < (int)cols() - border) {
...
template <class T>
void SmartGrid<T>::dilate(size_t radius) {
    // have to save a temporary copy of the depth map
    SmartGrid gridTmp(rows(), cols());
...
template <class T>
void SmartGrid<T>::clean(double var_threshold,
...
template <class T>
const T &SmartGrid<T>::operator()(size_t row, size_t col) const {
...
template <class T>
const T &SmartGrid<T>::at(size_t row, size_t col) const {
    if ((*_grid[row])[col] != NULL)
        return *((*_grid[row])[col]);
    return _invalid;
}
...
template <class T>
size_t SmartGrid<T>::rows() const {
    return _grid.size();
}

template <class T>
size_t SmartGrid<T>::cols() const {
    return (*_grid.front()).size();
}

template <class T>
void SmartGrid<T>::set(size_t row, size_t col, const T &value) {
    if ((*_grid[row])[col] == NULL) {
...
template <class T>
bool SmartGrid<T>::exists(size_t row, size_t col) {
    if ((*_grid[row])[col] == NULL)
        return false;
    return true;
}
...
template <class T>
T &SmartGrid<T>::get(size_t row, size_t col) {
    if ((*_grid[row])[col] == NULL)
        return _invalid;
    return *((*_grid[row])[col]);
}
...
template <class T>
typename SmartGrid<T>::gridElements::iterator SmartGrid<T>::begin() {
    return _elements.begin();
}

template <class T>
typename SmartGrid<T>::gridElements::iterator SmartGrid<T>::end() {
    return _elements.end();
}

template <class T>
size_t SmartGrid<T>::size() const {
    return _elements.size();
}

template <class T>
void SmartGrid<T>::getNeighbourhood(size_t            row,
                                    size_t            col,
                                    size_t            radius,
