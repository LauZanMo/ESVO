...
public:
    EventBM(CameraSystem::Ptr camSysPtr,
            size_t            numThread      = 1,
            ...
            bool bUpDownConfiguration = false); // bUpDownConfiguration determines the epipolar
                                                // searching direction (UpDown or LeftRight).
    virtual ~EventBM();

    void resetParameters(size_t patch_size_X,
                         size_t patch_size_Y,
    ...
    bool match_an_event(dvs_msgs::Event           *pEvent,
    ...
    static double
    zncc_cost(Eigen::MatrixXd &patch_left, Eigen::MatrixXd &patch_right, bool normalized = false);

private:
    void match(EventBM::Job &job);
    bool epipolarSearching(double          &min_cost,
