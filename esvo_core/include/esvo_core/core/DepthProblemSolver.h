...
public:
    ...
    DepthProblemSolver(
        ...
        DepthProblemType                     dpType =
            NUMERICAL, // we only provide the numerical (jacobian) solver here, because the
                       // analytical one will not show remarkable efficiency.
        size_t numThread = 1);

    virtual ~DepthProblemSolver();

    void solve(std::vector<EventMatchPair> *pvEMP,
    ...
    bool
    solve_single_problem_numerical(double                                               d_init,
                                   std::shared_ptr<Eigen::NumericalDiff<DepthProblem>> &dProblemPtr,
                                   double                                              *result);

    void pointCulling(std::vector<DepthPoint> &vdp,
