...
public:
    EventMatcher(CameraSystem::Ptr camSysPtr,
                 size_t            numThread                 = 1,
    ...
    void resetParameters(double Time_THRESHOLD,
    ...
    void createMatchProblem(StampedTimeSurfaceObs          *pTS_obs,
                            std::vector<EventSlice>        *vEventSlice_ptr,
                            std::vector<dvs_msgs::Event *> *vEventPtr_cand);

    bool
    match_an_event(dvs_msgs::Event *ev_ptr, Transformation &Trans_world_rv, EventMatchPair &emPair);
    ...
    bool warping2(const Eigen::Vector2d             &x,
                  double                             invDepth,
                  const Eigen::Matrix<double, 3, 4> &T_left_rv,
                  Eigen::Vector2d                   &x1_s,
                  Eigen::Vector2d                   &x2_s);

    bool patchInterpolation2(const Eigen::MatrixXd &img,
