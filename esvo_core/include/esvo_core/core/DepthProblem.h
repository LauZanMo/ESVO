...
struct DepthProblemConfig {
    ...
    DepthProblemConfig(size_t             patchSize_X,
                       size_t             patchSize_Y,
...
struct DepthProblem : public optimization::OptimizationFunctor<double> {
    ...
    // utils
    bool warping(const Eigen::Vector2d             &x,
                 double                             d,
    ...
    std::vector<Eigen::Matrix<double, 3, 4>, Eigen::aligned_allocator<Eigen::Matrix<double, 3, 4>>>
                           vT_left_virtual_;
